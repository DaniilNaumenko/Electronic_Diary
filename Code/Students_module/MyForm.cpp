...
void MyForm::insert() {
	...
	f2 << m.id << "\n";
	m.english = Convert::ToInt32(MyForm::textBox7->Text);
	f2 << m.english << "\n";

	m.russian = Convert::ToInt32(MyForm::textBox8->Text);
	f2 << m.russian << "\n";
...
void MyForm::scr_out(vector <Marks>::iterator p) {
	...
	richTextBox1->Text += "\nÎöåíêà ïî àíãëèéñêîìó: ";
	richTextBox1->Text += p->english;
	richTextBox1->Text += "\nÎöåíêà ïî ðóññêîìó: ";
	richTextBox1->Text += p->russian;
